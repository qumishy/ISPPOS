-- Hardened missing invoice_items repair for approved active/pending/non-high-risk invoices only.
-- Project: Supabase isp / ybpzjvswutvdbjevgawt
-- Prepared: 2026-06-23
--
-- Scope:
--   INV-2026-0640
...
--
-- Explicitly excluded:
--   INV-2026-0602, paid invoices, collected invoices, inactive/cancelled invoices,
--   ambiguous invoices, insufficient-stock invoices, unrecoverable invoices.
--
-- Schema confirmations from read-only schema audit:
--   public.collection_invoices exists.
--   public.operations_log has all columns used below.
--   public.agent_wallets.phase_id exists and is checked against invoice phase_id.
--   public.batches.phase_id does not exist, so no batch phase check is included.
--   public.invoice_items.total_price is generated and is not inserted.

-- ============================================================================
-- 1) DRY-RUN PRECHECK ONLY
-- ============================================================================
-- This section creates a temp table, inserts proposed repair rows into the temp
-- table only, locks affected wallets, runs all guards, shows before/proposed
-- verification, and rolls back. It does not insert/update/delete persistent data.
...
begin
  if (select count(*) from invoice_items_repair_20260623) <> 13 then
    raise exception 'Dry-run guard failed: expected 13 proposed rows';
  end if;

  if (select count(distinct invoice_id) from invoice_items_repair_20260623) <> 5 then
    raise exception 'Dry-run guard failed: expected exactly 5 invoices';
  end if;
  ...
  if exists (
    ...
    raise exception 'Dry-run guard failed: unapproved invoice included';
  end if;
  ...
  if exists (
    ...
    raise exception 'Dry-run guard failed: invoice is not active pending, already has items, or has collections';
  end if;
  ...
  if exists (
    ...
    raise exception 'Dry-run guard failed: wallet/category/batch/price/phase mismatch';
  end if;
  ...
  if exists (
    ...
    raise exception 'Dry-run guard failed: proposed item totals do not match invoice totals';
  end if;
  ...
  if exists (
    ...
    raise exception 'Dry-run guard failed: insufficient wallet stock';
  end if;
end $$;

select i.invoice_number,
       i.total_amount,
       sum(p.quantity * p.unit_price) as proposed_item_total,
...
select w.id as wallet_id,
       ...
       w.phase_id,
       w.total_cards,
       w.sold_cards as current_sold_cards,
       w.remaining_cards as current_remaining_cards,
       coalesce(p.proposed_qty, 0) as proposed_added_qty
from public.agent_wallets w
join (
  select wallet_id, sum(quantity) as proposed_qty
  from invoice_items_repair_20260623
...
-- ============================================================================
-- 2) FINAL EXECUTABLE REPAIR SQL
-- ============================================================================
-- Run this section only after the dry-run section passes and final approval is
-- given. It inserts invoice_items, recalculates affected wallet sold_cards from
-- active non-cancelled invoice_items using abs(quantity), logs to operations_log,
-- verifies the result, and commits.
...
-- 3) ROLLBACK SQL
-- ============================================================================
-- Use only if the committed repair must be reverted. It deletes only the fixed
-- repair item IDs from this file, then recalculates affected wallet sold_cards
-- from active non-cancelled invoice_items using abs(quantity).

begin;

create temp table rollback_invoice_item_ids_20260623 (
  id uuid primary key
) on commit drop;

insert into rollback_invoice_item_ids_20260623(id)
values
('00000000-0000-4000-9000-000000006401'),
('00000000-0000-4000-9000-000000006402'),
('00000000-0000-4000-9000-000000006403'),
('00000000-0000-4000-9000-000000006451'),
('00000000-0000-4000-9000-000000006452'),
('00000000-0000-4000-9000-000000006453'),
('00000000-0000-4000-9000-000000006471'),
('00000000-0000-4000-9000-000000006472'),
('00000000-0000-4000-9000-000000006473'),
('00000000-0000-4000-9000-000000006481'),
('00000000-0000-4000-9000-000000006482'),
('00000000-0000-4000-9000-000000006483'),
('00000000-0000-4000-9000-000000006491');

create temp table affected_wallets_rollback_20260623 on commit drop as
select distinct wallet_id
from public.invoice_items
where id in (select id from rollback_invoice_item_ids_20260623);

select w.id as locked_wallet_id
from public.agent_wallets w
join affected_wallets_rollback_20260623 aw on aw.wallet_id = w.id
for update;

delete from public.invoice_items
where id in (select id from rollback_invoice_item_ids_20260623);

with active_item_sold as (
  select ii.wallet_id, coalesce(sum(abs(ii.quantity)), 0)::integer as sold_cards
  from public.invoice_items ii
  join public.invoices i on i.id = ii.invoice_id
  where ii.wallet_id in (select wallet_id from affected_wallets_rollback_20260623)
    and coalesce(i.active, true) = true
    and coalesce(i.status, 'pending') not in ('cancelled', 'canceled', 'rejected', 'deleted')
  group by ii.wallet_id
)
update public.agent_wallets w
set sold_cards = coalesce(s.sold_cards, 0)
from affected_wallets_rollback_20260623 aw
left join active_item_sold s on s.wallet_id = aw.wallet_id
where w.id = aw.wallet_id;

select w.id as wallet_id, w.total_cards, w.sold_cards, w.remaining_cards
from public.agent_wallets w
where w.id in (select wallet_id from affected_wallets_rollback_20260623)
order by w.id;
