...
export default function CollectionsScreen({ navigation }) {
  ...
  const metaChip = {
    flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'space-between', backgroundColor: colors.bg2,
    borderWidth: 1, borderColor: colors.border + '80', borderRadius: radius.md, paddingHorizontal: spacing.sm,
    paddingVertical: 7, minWidth: '48%', flex: 1,
  };
  const sectionBox = { backgroundColor: colors.bg2 + '90', borderWidth: 1, borderColor: colors.border + '70', borderRadius: radius.lg, padding: spacing.sm, gap: 6 };
  ...
  const statusChip = { flexDirection: 'row-reverse', justifyContent: 'space-between', alignItems: 'center', backgroundColor: colors.bg2, borderWidth: 1, borderColor: colors.border + '80', borderRadius: radius.md, paddingHorizontal: spacing.sm, paddingVertical: 6, gap: 8 };

  const invoiceApprovalDisplayMeta = (status) => {
    const meta = invoiceApprovalStatusMeta(status);
    const normalized = normalizeStatus(status);
    if (['pending', 'pending_collection_approval', 'pending_card_return_approval', 'unapproved', 'not_approved'].includes(normalized)) {
  ...
  const handlePrint = async (col) => {
    const html = `<html dir="rtl" lang="ar"><body><h2>سند قبض</h2><div>رقم السند: ${col.collection_number}</div><div>التاريخ: ${col.collection_date}</div><div>العميل: ${col.pos_name}</div><div>طريقة الدفع: ${methodLabel(col.method)}</div><div>المندوب: ${col.agent_name}</div><div>المبلغ: ${formatCurrency(col.amount)}</div></body></html>`;
  ...
  const generateReceiptInfo = (col) => {
    ...
    if (net > 0) return remaining <= 0.1 ? '✅ تم استكمال سداد الفاتورة بالكامل.' : `المتبقي من الفاتورة: ${formatCurrency(remaining)}`;
    return '';
  };
  ...
  const handleWhatsApp = (col) => {
    ...
    Linking.canOpenURL(url).then(supp => supp ? Linking.openURL(url) : Alert.alert('خطأ', 'تطبيق واتساب غير مثبت'));
  };
      ...
      <Modal visible={showApproveModal} transparent animationType="fade">
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', padding: 20 }}>
          <View style={{ backgroundColor: colors.card, padding: 20, borderRadius: radius.lg }}>
            ...
            <TextInput style={{ backgroundColor: colors.bg2, padding: 12, borderRadius: radius.md, minHeight: 80, textAlignVertical: 'top', color: colors.t1 }} placeholder="اكتب أي ملاحظات هنا (اختياري)..." value={approveNotes} onChangeText={setApproveNotes} multiline />
            <Row style={{ gap: 10, marginTop: 20 }}><Btn label="إلغاء" variant="outline" style={{ flex: 1 }} onPress={() => setShowApproveModal(false)} /><Btn label="تأكيد الاعتماد" variant="primary" style={{ flex: 1 }} onPress={confirmApprove} /></Row>
          </View>
        </View>
      </Modal>

      <ScreenHeader
        kpis={[{ label: 'معلق', value: pending.length, color: colors.warning }, { label: 'قيد الانتظار', value: formatCurrency(totalPending), color: colors.warning }, { label: 'محصّل', value: formatCurrency(totalApproved), color: colors.success }]}
        tabs={[{ k: 'pending', l: `معلقة (${pending.length})` }, { k: 'approved', l: `معتمدة (${approved.length})` }, { k: 'all', l: `الكل (${visibleCollections.length})` }]}
        activeTab={tab} onTabSelect={setTab} search={search} onSearch={setSearch} searchPlaceholder="بحث بالرقم أو الاسم..."
        action={selectedPhase?.status !== 'closed' ? "+ سند" : undefined} onAction={selectedPhase?.status !== 'closed' ? () => navigation.push('NewCollection') : undefined} onFilter={() => setShowFiltersModal(true)}
      />

      <AdvancedFiltersModal visible={showFiltersModal} onClose={() => setShowFiltersModal(false)} currentFilters={advancedFilters} onApply={(f) => setAdvancedFilters(f)} type="collections" />
      ...
      {loading && !hasDisplayedLocalDataRef.current ? <Loading /> : filtered.length === 0
        ...
        : <FlatList
          ...
          renderItem={({ item: col }) => {
            ...
            return (
              <TouchableOpacity style={s.colCard} activeOpacity={0.85} onPress={() => setExpandedColId(expanded ? null : col.id)}>
                <View style={{ flexDirection: 'row-reverse', justifyContent: 'space-between', gap: 10 }}>
                  <View style={{ flex: 1 }}><Text style={s.colNum}>{col.collection_number || '—'}</Text><Text style={s.invPos}>{col.pos_name || '—'}</Text></View>
                  <View style={{ alignItems: 'flex-end' }}><Text style={s.colAmt}>{formatCurrency(collectionAmount)}</Text><Text>{formatDateShort(col.collection_date)}</Text></View>
                </View>
                ...
                <View style={{ flexDirection: 'row-reverse', flexWrap: 'wrap', gap: 8, marginTop: 8 }}>
                  <View style={metaChip}><Text style={labelMini}>مبلغ التحصيل</Text><Text style={valueMini}>{formatCurrency(collectionAmount)}</Text></View>
                  <View style={metaChip}><Text style={labelMini}>مرتجع الكروت</Text><Text style={valueMini}>{formatCurrency(linkedReturnAmount)}</Text></View>
                  <View style={metaChip}><Text style={labelMini}>الفاتورة</Text><Text style={valueMini}>{col.invoice_number || '-'}</Text></View>
                  <View style={metaChip}><Text style={labelMini}>طريقة الدفع</Text><Text style={valueMini}>{methodLabel(col.method)}</Text></View>
                </View>
                  ...
                  <>
                    <View style={{ marginTop: 8, paddingTop: 8, borderTopWidth: 1, borderTopColor: colors.border + '30' }}>
                      <View style={s.colGrid}>
                        {[col.agent_name && { label: 'المندوب', value: col.agent_name }, { label: 'طريقة الدفع', value: methodLabel(col.method) }, isAdminUser && isApprovedCollectionRow(col) && col.approver_name && { label: 'المعتمد', value: col.approver_name, color: colors.warning }, col.approval_notes && { label: 'ملاحظات', value: col.approval_notes, color: colors.success }].filter(Boolean).map((item, i) => (
                          <View key={i} style={s.colGridItem}><Text style={s.colGridLabel}>{item.label}</Text><Text style={[s.colGridVal, item.color && { color: item.color }]}>{item.value}</Text></View>
                        ))}
                      </View>
                    </View>
                    <View style={[s.colActions, { paddingVertical: 8, gap: 10 }]}>
                      <Row style={{ gap: 10 }}>
                        ...
                        {selectedPhase?.status !== 'closed' && canApproveCollectionRow && <Btn label="اعتماد" icon="check-circle" variant="success" size="sm" style={{ flex: 1 }} onPress={() => handleApprovePress(col.id)} />}
                        {selectedPhase?.status !== 'closed' && isApprovedCollectionRow(col) && isAdminUser && <Btn label="إلغاء التحصيل" icon="x-circle" variant="danger" size="sm" style={{ flex: 1 }} onPress={() => handleCancelApproval(col.id)} />}
                      </Row>
                    </View>
                  </>
                )}
