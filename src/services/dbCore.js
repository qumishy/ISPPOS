      ...
      tx => {
        ...
        function step(resumeValue) {
          try {
            ...
            if (done) {
              finalResult = value;
              return; // transaction will commit automatically
            }
            if (!value || !value.sql) {
...
export const addToSyncQueue = async (tableName, operation, payload, recordId = null) => {
  ...
  const ins = await execSQL(
    `INSERT INTO sync_queue (table_name, operation, payload, record_id, project_id, attempts, created_at)
     VALUES (?, ?, ?, ?, ?, 0, datetime('now'))`,
    [tableName, operation, normalizedPayload, recordId, projectId]
  );
  try {
...
const createTables = async () => {
  ...
  await execSQL(`CREATE TABLE IF NOT EXISTS sync_queue (id INTEGER PRIMARY KEY AUTOINCREMENT, table_name TEXT NOT NULL, operation TEXT NOT NULL, payload TEXT, record_id TEXT, attempts INTEGER DEFAULT 0, created_at TEXT DEFAULT CURRENT_TIMESTAMP, project_id TEXT)`);
  await execSQL(`CREATE TABLE IF NOT EXISTS operations_log (id TEXT PRIMARY KEY NOT NULL, sync_queue_id INTEGER, actor_user_id TEXT, actor_name TEXT, actor_role TEXT, operation_type TEXT NOT NULL, table_name TEXT NOT NULL, entity_name TEXT, record_id TEXT, reference_text TEXT, message_ar TEXT NOT NULL, old_values TEXT, new_values TEXT, project_id TEXT, phase_id TEXT, source TEXT DEFAULT 'sqlite', sync_status TEXT DEFAULT 'pending', sync_error TEXT, sync_details TEXT, device_id TEXT, session_id TEXT, created_at TEXT DEFAULT CURRENT_TIMESTAMP, updated_at TEXT DEFAULT CURRENT_TIMESTAMP, synced_at TEXT)`);
  ...
  await execSQL(`CREATE TABLE IF NOT EXISTS app_notifications (id TEXT PRIMARY KEY NOT NULL, project_id TEXT, user_id TEXT, title TEXT, body TEXT, type TEXT, reference_id TEXT, route TEXT, params TEXT, is_read INTEGER DEFAULT 0, created_at TEXT)`);
...
const resetDatabaseOnce = async () => {
  try {
    ...
    if (!initialized) {
      const tables = ['users', 'pos_customers', 'card_categories', 'batches', 'invoices', 'invoice_items', 'collections', 'agent_wallets', 'sync_queue', 'sync_meta', 'supplies', 'invoice_notifications_log', 'app_permissions', 'project', 'phases', 'operations_log'];
